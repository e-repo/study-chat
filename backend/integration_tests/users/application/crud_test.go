...
func (s *UsersSuite) TestHTTP() {
	...
	s.Run("Create", func() {
		...
		s.Require().NoError(err, "Failed to create users via HTTP")
		s.Require().Equal(201, createResp.StatusCode, "Failed to create users via HTTP")
	...
	s.Run("Get", func() {
		...
		s.Require().NoError(err, "Failed to get users via HTTP")
		s.Require().Equal(200, getResp.StatusCode, "Failed to get users via HTTP")
